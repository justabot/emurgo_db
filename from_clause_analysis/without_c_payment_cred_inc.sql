  ...
  --       )
  --       )
         WHERE source_tx_out.address = ANY(('{}')::varchar array)
               OR source_tx_out.payment_cred = ANY(('{\\x220099e5e430475c219518179efc7e6c8289db028904834025d5b086}')::bytea array)
          UNION
        select tx.hash as hash
            FROM tx
  ...
  --       )
  --       )
         WHERE  tx_out.address = ANY(('{}')::varchar array)
or tx_out.payment_cred = ANY(('{\\x220099e5e430475c219518179efc7e6c8289db028904834025d5b086}')::bytea array)
          UNION
            select tx.hash as hash
            from tx
  ...
  where
        block.block_no <= 7000000
        and (
          block.block_no > 6999000
            or
          (block.block_no = 6999000 and tx.block_index > 15)
        )
