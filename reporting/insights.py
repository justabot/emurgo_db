...
# del_cmd = """
# DELETE FROM reporting.insights WHERE outlink_date >= '%(d)s'::timestamptz;
# """ % {"d": request_date}
# cur.execute(del_cmd)
# conn.commit()
now = datetime.now()
while request_date <= now.date():
  url = "https://xxx/";
  url += "?module=API&method=Insights.getInsights";
  url += "&idSite=1&period=day&date=%(d)s"% {"d": request_date};
  url += "&reportUniqueId=1337&format=JSON&filter_limit=10";
